...
def scal_rel_abun(dp,rmOthers=True):
    ...
    #df_log10.dropna(axis=1, how='any',inplace=True)
    
    
    ## log2
    df_log2=df.apply(np.log2)
    #df_log2_norm = (df_log2 - df_log2.min()) / (df_log2.max() - df_log2.min())
    df_log2.replace([np.inf, -np.inf], np.nan, inplace=True)
    #df_log2.dropna(axis=1, how='any',inplace=True)
    ...
    output_log10=str(dp+".log10"+suffix1+suffix2+".csv")
    df_log10.to_csv(output_log10,index=True,sep=',')
    output_log2=str(dp+".log2"+suffix1+suffix2+".csv")
    df_log2.to_csv(output_log2,index=True,sep=',')
    output_norm=str(dp+".norm"+suffix1+suffix2+".csv")
...
if __name__=='__main__':
    for i in range(1,8):
        ...
        dp = r"D:\CurrentProjects\equids_MHC\Prj1\viruses\sample18_rel_abun.{}.rmU.euk.csv".format(i)
        try:
            scal_rel_abun(dp)
        except:
