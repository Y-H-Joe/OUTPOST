...
def res2plot(dp,LDA_cutoff,output):
    ...
    # Create barplot
    left = min(round(min(bars1+bars2),1),LDA_cutoff)
...
if __name__=='__main__':
    #for i in range(1,9):
    for i in ['eggnog','ko','level4ec','pfam','rxn']:
        #dp=r'C:\CurrentProjects\equids_MHC\Prj1\lefse\sample12_rel_abun.{}.rmU.euk.csv.lefse.tsv.format.res'.format(i)
        dp=r'C:\CurrentProjects\equids_MHC\Prj1\lefse\horsedonkey_genefamilies_uniref90names_cpm_{}_unstratified.named.tsv.rel_abun.csv.lefse.tsv.format.res'.format(i)
        output=str(dp+".barh.pdf")
        LDA_cutoff=3
        try:
            res2plot(dp,LDA_cutoff,output)
        except:
            print(dp," has no features.")
