...
sample_list, refgenome_list, r1_fq_list, r2_fq_list, se_fq_list, bam_dir_list, assembly_list, assembly_dir_list, batch_list, comparison_dict = check_config(df_config, rm_batch_effect)
...
if len(r1_fq_list + se_fq_list) > 0:
    ...
    rule metaphlan_krona:
        ...
        run:
            ...
            for i, sample in enumerate(sample_list):
                ...
                shell("{python}  OUTPOST/outpsot_metaphlan2krona.py -p {metaphlan_output} -k {krona_tsv}" )
            ...
            shell("{ktimporttext} ' '.join(krona_tsvs)  -o {krona_plot} ")
    ...
    rule metaphlan_graphlan:
        ...
        run:
            ...
            shell("{python} {export2graphlan} --skip_rows 0 -i {output_merged} \
            ...
            shell("{graphlan} {graphlan_xml} {graphlan_pdf} ")
    ...
    rule metaphlan_heatmap:
        ...
        run:
            ...
            shell("{python} OUTPOST/metaphlan_hclust2.py -i {output_merged} \
    ...
    rule metaphlan_diverisy:
        ...
        run:
            # beta diversity
            shell("{Rscript} OUTPOST/metaphlan_calculate_diversity.R \
    ...
    rule metaphlan_init:
        ...
        run:
            ...
            # reformat to tsv
            shell("{python} OUTPOST/taxa_annatation2tsv.py {output_merged} {','.join(sample_list)} \
        ...
        rule humann_init:
            ...
            run:
                ...
                for fq_batch in fq_list_batch:
                    if len(fq_batch) == 1:
                        fq = fq_batch[0]
                        shell("{humann} --resume --input {fq}  --output {output_dir}/function_analysis/humann3/ori_results/ "
                        " --search-mode uniref90 --diamond-options '--block-size 10 --fast' "
                        " --threads {threads} --memory-use maximum > {log} 2>&1 ")
                    elif len(fq_batch) > 1:
                        for fq in fq_batch[:-1]:
                            shell("nohup {humann} --resume --input {fq}  --output {output_dir}/function_analysis/humann3/ori_results/ "
                            " --search-mode uniref90 --diamond-options '--block-size 10 --fast' "
                            " --threads {threads} --memory-use maximum > {log} 2>&1 &")
                        fq = fq_batch[-1]
                        shell("{humann} --resume --input {fq}  --output {output_dir}/function_analysis/humann3/ori_results/ "
...
if len(assembly_dir_list) > 0:
            ...
            for assembly,assembly_dir in set(zip(assembly_list,assembly_dir_list)):
                if assembly != '':
                    ...
                    shell("{emapper} --data_dir {emapper_db} \
                          ...
                          --output_dir {eggmapper_annotate_prodigal_dir} --output outpost --resume")
                    
                    metagenemark_dir = f"{output_dir}/assembly_analysis/annotation/metagenemark"
                    shell("{emapper} --data_dir {emapper_db} \
                          -i {prodigal_dir}/{assembly}_metagenemark_prot.fa --cpu {threads} -m diamond \
                          --output_dir {eggmapper_annotate_metagenemark_dir}  --output outpost --resume")
    ...
    rule rgi_gene_annotation:
        ...
        threads: cores
        run:
            rgi_annotate_prodigal_dir = f"{output_dir}/assembly_analysis/annotation/rgi/prodigal"
            rgi_annotate_metagenemark_dir = f"{output_dir}/assembly_analysis/annotation/rgi/metagenemark"
            os.makedirs(rgi_annotate_prodigal_dir, exist_ok=True)
            os.makedirs(rgi_annotate_metagenemark_dir, exist_ok=True)
            for assembly,assembly_dir in set(zip(assembly_list,assembly_dir_list)):
                if assembly != '':
                    prodigal_dir = f"{output_dir}/assembly_analysis/annotation/prodigal"
                    shell("{rgi} main -i {prodigal_dir}/{assembly}_prodigal_gene.fa -n {threads} \
                          --include_loose  --local  --clean -o {rgi_annotate_prodigal_dir} ")
                          
                    metagenemark_dir = f"{output_dir}/assembly_analysis/annotation/metagenemark"
                    shell("{rgi} main -i {metagenemark_dir}/{assembly}_metagenemark_gene.nonrd.fa -n {threads} \
                          --include_loose  --local  --clean -o {rgi_annotate_metagenemark_dir} ")
    ...
    if (len(r1_fq_list + se_fq_list) > 0) and assemble_contigs:
        ...
        gtdbtk_genome_annotation_input = "{output_dir}/log/quast_contigs.done"
    else:
        metagenemark_input = assembly_dir_list
        prodigal_gene_prediction_input = assembly_dir_list
        gtdbtk_genome_annotation_input = assembly_dir_list
        
        
    #%% gtdbtk
    rule gtdbtk_genome_annotation:
        input:
            gtdbtk_genome_annotation_input
        output:
        ...
        run:
            ...
            for assembly,assembly_dir in set(zip(assembly_list,assembly_dir_list)):
                if assembly != '':
                    ...
                    shell("gtdbtk classify_wf --genome_dir {assembly_dir} --out_dir {gtdbtk_dir} \
    ...
    rule metagenemark:
        ...
        threads: cores
        run:
            output_dir = f"{output_dir}/assembly_analysis/annotation/metagenemark"
            os.makedirs(output_dir, exist_ok=True)
            for assembly,assembly_dir in set(zip(assembly_list,assembly_dir_list)):
                if assembly != '':
                    # predict genes
                    shell("{mgm} -r -p 1 -m {mod_file} -k -K {output_dir}/{assembly}_metagenemark.rbs -f G -o {output_dir}/{assembly}_metagenemark.gtf -a -A {output_dir}/{assembly}_metagenemark_prot.fa -d -D {output_dir}/{assembly}_metagenemark_gene.fa {assembly_dir}")
                    # remove redundancy
                    shell("{cdhit} -i {output_dir}/{assembly}_metagenemark_gene.fa -o {output_dir}/{assembly}_metagenemark_gene.nonrd.fa -c {cdhit_cutoff} -n 10 -M 0 -T {cores}")
    ...
    if (len(r1_fq_list + se_fq_list) > 0 and len(assembly_list) > 0) or (len(bam_dir_list) > 0):
            ...
            run:
                ...
                for level in taxa_level:
                    for group1,group2 in group_pair_list:
                        ...
                        # taxa_abun_top
                        try:
                            dp = f"{output_dir}/taxonomy_analysis/{assembly}.taxa_counts.rel_abun.{level}.rmU.csv"
                            ...
                            pass
                        # taxa_abun_signicant
                        dp = f"{output_dir}/taxonomy_analysis/utest_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.u-test." +"paired."*paired + "two_sided."*two_sided + "csv"
                        tmp = pd.read_csv(dp, index_col=0)
                        ...
                        # LDA_larger_than
                        dp1 = f"{output_dir}/LDA_analysis/taxa_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.unequal.lefse.res"
                        dp2 = f"{output_dir}/LDA_analysis/taxa_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.equal.lefse.res"
                        dp1_exists = os.path.exists(dp1)
                        dp2_exists = os.path.exists(dp2)
                        dp1 = dp1 * dp1_exists
                        dp2 = dp2 * dp2_exists
                        dp = f"{output_dir}/LDA_analysis/taxa_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.lefse.res"
                        try:
        ...
        rule plasmids_analysis_assembly_based:
            ...
            run:
                output_dir = f"{output_dir}/plasmids_analysis/assembly_based"
                os.makedirs(output_dir, exist_ok=True)
                for assembly,assembly_dir in set(zip(assembly_list,assembly_dir_list)):
                    if assembly != '':
                        ...
                        for group1,group2 in group_pair_list:
                            ...
                            shell("{python3} OUTPOST/visualize_abricate.py {output_dir}/{assembly}.plasmidfinder.tsv {output_dir}/assembly_analysis/taxa_counts/all_samples.taxa_counts.tsv"
        ...
        rule virulence_factors_analysis_assembly_based:
            ...
            run:
                output_dir = f"{output_dir}/virulence_factors_analysis/assembly_based"
                os.makedirs(output_dir, exist_ok=True)
                for assembly,assembly_dir in set(zip(assembly_list,assembly_dir_list)):
                    if assembly != '':
                        ...
                        for group1,group2 in group_pair_list:
                            group1_index = ','.join([str(sample_list.index(sample)) for sample in comparison_dict[group1]])
                            group2_index = ','.join([str(sample_list.index(sample)) for sample in comparison_dict[group2]])
                            shell("{python3} OUTPOST/visualize_abricate.py {output_dir}/{assembly}.virulence.tsv {output_dir}/assembly_analysis/taxa_counts/all_samples.taxa_counts.tsv "
        ...
        rule antibiotic_genes_analysis_assembly_based:
            ...
            run:
                output_dir = f"{output_dir}/antibiotic_genes_analysis/assembly_based"
                os.makedirs(output_dir, exist_ok=True)
                for assembly,assembly_dir in set(zip(assembly_list,assembly_dir_list)):
                    if assembly != '':
                        ...
                        for group1,group2 in group_pair_list:
                            group1_index = ','.join([str(sample_list.index(sample)) for sample in comparison_dict[group1]])
                            group2_index = ','.join([str(sample_list.index(sample)) for sample in comparison_dict[group2]])
                            shell("{python3} OUTPOST/visualize_abricate.py {output_dir}/{assembly}.antibiotic.tsv {output_dir}/assembly_analysis/taxa_counts/all_samples.taxa_counts.tsv "
                                  " {output_dir} {group1_index} {group1} {group2_index} {group2} {taxa_level_temp}")
        ...
        rule alpha_beta_diversity:
            ...
            run:
                for group1,group2 in group_pair_list:
                    ...
                    species_dp = f"{output_dir}/taxonomy_analysis/{assembly}.taxa_counts.rel_abun.{species}.rmU.csv"
                    genus_dp = f"{output_dir}/taxonomy_analysis/{assembly}.taxa_counts.rel_abun.{genus}.rmU.csv"
        ...
        rule visualize_batch_effect:
            ...
            run:
                ...
                for level in taxa_level:
                    ...
                    if rm_batch_effect:
                        plot = f"{output_dir}/batch_effect/all_samples.taxa_counts.rel_abun.{level}.rmU.batch_effect_PCA.pdf"
                    else:
                        plot = f"{output_dir}/batch_effect/all_samples.taxa_counts.rel_abun.{level}.rmU.not_rm_batch_effect_PCA.pdf"
        ...
        rule heatmap_taxa:
            ...
            run:
                ...
                data_type = 'taxa'
                for level in taxa_level:
                    dp = f"{output_dir}/taxonomy_analysis/{assembly}.taxa_counts.rel_abun.{level}.rmU.top{top}.fillmin.scaled.csv"
                    output = os.path.basename(dp).replace(".csv",".heatmap.pdf")
                    index_dp = f"{output_dir}/taxonomy_analysis/{assembly}.taxa_counts.rel_abun.{level}.rmU.top{top}.fillmin.scaled.index"
                    with open(index_dp,'w') as w:
                        for sample in sample_list:
                            w.write(sample + '\n')
                    try:
                ...
                for group1,group2 in group_pair_list:
                    for level in taxa_level:
                        dp = f"{output_dir}/taxonomy_analysis/top_taxa_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.unequal.top{top}.fillmin.scaled.csv"
                        output = os.path.basename(dp).replace(".csv",".heatmap.pdf")
                        index_dp = f"{output_dir}/taxonomy_analysis/top_taxa_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.unequal.top{top}.fillmin.scaled.index"
                        with open(index_dp,'w') as w:
                            for sample in comparison_dict[group1]:
                                w.write(f"{sample}_{group1}\n")
                            for sample in comparison_dict[group2]:
                ...
                for group1,group2 in group_pair_list:
                    for level in taxa_level:
                        dp = f"{output_dir}/taxonomy_analysis/top_taxa_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.equal.top{top}.fillmin.scaled.csv"
                        output = os.path.basename(dp).replace(".csv",".heatmap.pdf")
                        index_dp = f"{output_dir}/taxonomy_analysis/top_taxa_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.equal.top{30}.fillmin.scaled.index"
                        with open(index_dp,'w') as w:
                            for sample in comparison_dict[group1]:
                                w.write(f"{sample}_{group1}\n")
                            for sample in comparison_dict[group2]:
        ...
        rule lefse_taxa:
            ...
            run:
                ...
                for group1,group2 in group_pair_list:
                    dp_list = [f"{output_dir}/LDA_analysis/taxa_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.unequal.lefse.tsv"
                               for level in taxa_level]
                    format_list = [dp.replace(".tsv",".format") for dp in dp_list]
                    res_list = [dp.replace(".tsv",".res") for dp in dp_list]
                    output_list = [wkdir + os.path.basename(dp).replace(".tsv",".pdf") for dp in dp_list]
                ...
                for group1,group2 in group_pair_list[:-2]:
                    dp_list = [f"{output_dir}/LDA_analysis/taxa_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.equal.lefse.tsv"
                               for level in taxa_level]
                    format_list = [dp.replace(".tsv",".format") for dp in dp_list]
                    res_list = [dp.replace(".tsv",".res") for dp in dp_list]
                    output_list = [wkdir + os.path.basename(dp).replace(".tsv",".pdf") for dp in dp_list]
        ...
        rule rel_abun2lefse_taxa:
            ...
            run:
                ...
                for group1,group2 in group_pair_list:
                    ...
                    os.makedirs(wkdir, exist_ok=True)
                    dp_list = ','.join([f"{output_dir}/taxonomy_analysis/utest_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.unequal.csv"
                                        for level in taxa_level])
                    output_list = ','.join([wkdir + os.path.basename(dp).replace(".csv",".lefse.tsv")
                                        for dp in dp_list.split(',')])
                    class_ = ','.join([group1] * len(comparison_dict[group1]) + [group2] * len(comparison_dict[group2]))
                ...
                for group1,group2 in group_pair_list:
                    wkdir = f"{output_dir}/LDA_analysis/taxa_{group1}_vs_{group2}/"
                    os.makedirs(wkdir, exist_ok=True)
                    dp_list = ','.join([f"{output_dir}/taxonomy_analysis/utest_{group1}_vs_{group2}/{assembly}.rel_abun.{group1}_vs_{group2}.at_{level}.rel_abun.equal.csv"
                                        for level in taxa_level])
                    output_list = ','.join([wkdir + os.path.basename(dp).replace(".csv",".lefse.tsv")
                                        for dp in dp_list.split(',')])
                    class_ = ','.join([group1] * len(comparison_dict[group1]) + [group2] * len(comparison_dict[group2]))
...
if (len(r1_fq_list + se_fq_list) > 0 and len(assembly_list) > 0) or (len(bam_dir_list) > 0):
    rule paste_counts_table:
        ...
        run:
            ...
            shell("{python3} OUTPOST/paste_counts_table.py 3 {counts_table_list} {output_dir}/taxonomy_analysis/temp/all_samples.counts.pure {','.join(batch_list)} {rm_batch_effect} ")
...
if len(r1_fq_list + se_fq_list) > 0 and len(assembly_list) > 0:
    if assemble_contigs:
        align2assembly_input = [f"{output_dir}/log/outpost_contigs.done","{output_dir}/log/reads_QC.done"]
    else:
        align2assembly_input = f"{output_dir}/log/reads_QC.done"
    rule align2assembly:
        input:
            align2assembly_input
        output:
            "{output_dir}/log/align2assembly.done"
        log:
        ...
        run:
            for i, sample in enumerate(sample_list):
                ...
                if assembly != '':
                    output_sam = f"{output_dir}/data/{sample}align2{assembly}_unsorted.sam"
                    output_bam = f"{output_dir}/data/{sample}align2{assembly}_unsorted.bam"
                    output_bam1 = f"{output_dir}/data/{sample}align2{assembly}_sorted.bam"
...
if len(r1_fq_list + se_fq_list) > 0:
    if assemble_contigs:
        ...
        rule outpost_contigs:
            ...
            benchmark:
                "{output_dir}/benchmark/outpost_contig.benchmark"
            threads: cores
            run:
                ...
                # remove redundancy
                nonrd_outpost_contig = f"{output_dir}/assembly_analysis/outpost_contigs/outpost_nonrd_contigs.fasta"
                shell("{cdhit} -i {contig_dp} -o {nonrd_outpost_contigs} -c {cdhit_cutoff} -n 10 -M 0 -T {threads}")
                shell("cat {nonrd_outpost_contigs} | {seqkit} seq -w 60 > {outpost_contig}.tmp")
                shell("mv {outpost_contig}.tmp {outpost_contig}")
        ...
        rule assemble_contigs:
            ...
            run:
                ...
                if assembly_method == 'megahit':
                    command_megahit = "-1 " + ",".join(r1_fq_list) + " -2 " + ",".join(r2_fq_list)
            ...
            run:
                ...
                for i, sample in enumerate(sample_list):
                    ...
                    if len(refgenome_list) > 0:
                        ...
                        nonvirus_r12 = f"{output_dir}/data/{sample}_nonhostvirus.fq"
                        ...
                        shell("{trimmomatic} PE -phred33 {r1} {r2} \
                         ...
                         ILLUMINACLIP:{adapters}:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:32 2 \
                        > {trimm_log} ")
                        ...
                        # prepare humann
                        shell("cat {nonvirus_r1} {nonvirus_r2} > {nonvirus_r12}")
                    else:
                        shell("cat {r1} {f2} > {nonvirus_r12}")
                shell("touch {output_dir}/log/reads_QC_PE.done")
                if len(se_fq_list) > 0:
                    if os.path.exists(r"{output_dir}/log/reads_QC_SE.done"):
                        shell("touch {output_dir}/log/reads_QC.done")
                else:
                    shell("touch {output_dir}/log/reads_QC.done")
        ...
        rule downsample_PE:
            ...
            run:
                ...
                for i,sample in enumerate(sample_list):
                    ...
                    json = f"{assembly}/qc/{sample}.fastp.json"
                    html = f"{assembly}/qc/{sample}.fastp.html"
            ...
            run:
                ...
                for i, sample in enumerate(sample_list):
                    ...
                    if len(refgenome_list) > 0:
                        ...
                        nonvirus_se = f"{output_dir}/data/{sample}_nonhostvirus.fq"
                ...
                if len(r1_fq_list) > 0:
                    if os.path.exists(r"{output_dir}/log/reads_QC_PE.done"):
                        shell("touch {output_dir}/log/reads_QC.done")
                else:
                    shell("touch {output_dir}/log/reads_QC.done")
